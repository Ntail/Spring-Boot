...
/**
 * 面向切面编程 主要功能：日志记录，性能统计，安全控制，事物处理，异常处理等
 */
@Component
@Aspect
// 使用order对多个切面进行排序，参数越小，越在前
@Order(1)
public class LoggerHandlerAop {

    public static final Logger logger = LoggerFactory.getLogger(LoggerHandlerAop.class);
    ...
    // 匹配任何返回值类型的cn.timebusker包及任何子包下的以add开头的参数为String类型的方法执行
    @Pointcut(value = "execution(* cn.timebusker..add*(String))")
    public void log6(){};

    // 匹配任何返回值类型的cn.timebusker.service包及任何子包下的以add开头的参数为int类型的方法执行
     ...
     * 2.注意是AOP代理对象的类型匹配，这样就可以包括引入接口方法也可以匹配，注意this中使用的表达式必须是类型全限定名，不支持通配符
     */
    // 匹配指定类型内的方法执行(包下所有的类)
    ...
    /**
     * 四:targer(类型全限定名)--匹配当前目标对象类型的执行方法
     * 1.可以直接匹配接口类型完成，类型全名限定匹配
     * 2.注意是目标对象的类型匹配，这样就不包括引入接口也类型匹配，：注意targer中使用的表达式必须是类型全限定名，不支持通配符
     */
    // 匹配指定类型内的方法执行(包下所有的类)
    public void logt2(){}
     ...
     * 1.注意是匹配传入的参数类型，不是匹配方法签名的参数类型；参数类型列表中的参数必须是类型全限定名，通配符不知处
     * 2.args属于动态切入点，是在运行时动态匹配的，这种切入点开销非常大，非特殊情况最好不要使用
     */

    /**
     * 六：@within(注解类型)--匹配所有持有指定注解类型内的方法；注解类型也必须是全限定类型名
     * 1.注解类型也必须是全限定类型名
     */
    ...
    public Object aroundAdvice(ProceedingJoinPoint pjp){
        ...
        try{
            ...
            obj = pjp.proceed();
            System.out.printf("MoocAspect around 2.");
        } catch (Throwable e){
            e.printStackTrace();
        }
        return obj;
